...
public class Subtask2 {
    public static void main(String[] args) {
        ...
        double number = console.nextDouble(); //181232375
        int numberToInt = (int) number;
        double tmp = number % numberToInt;
        if (number % numberToInt == 0){
            long result = 1;
            int digits;
            String numberString = Integer.toString(numberToInt);

            System.out.println(numberString.length());

            for (int i = (numberString.length() - 1); i >= 0; i--) {
                if (i >= 1) {
                    digits = (int) number / ((int) Math.pow(10, i));
